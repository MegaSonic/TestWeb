...
var TCube = TObject.subclass('users.TCube',
// users.TCube is a demo object to illustrate the user of TObject
  'initalizing', {
    initialize: function($super, parent, onComplete, w, h, d, dw, dh, dd){
...
var TTeapot = TObject.subclass('users.TTeapot',
// users.TCube is a demo object to illustrate the user of TObject
  'initalizing', {
    initialize: function($super, parent, onComplete, size){
...
var TRectangle = TObject.subclass('users.TRectangle',
// users.TCube is a demo object to illustrate the user of TObject
  'properties',{
    width: 8,
    height: 8
  ...
  'initalizing', {
    initialize: function($super, parent, onComplete, w, h, wsegs, hsegs){
      this.width = w ||8;
      this.height = h || 8;
...
var TTexturedRectangle = TObject.subclass('users.TTexturedRectangle',
// users.TCube is a demo object to illustrate the user of TObject
  'initalizing', {
    initialize: function($super, parent, onComplete, textureName, w, h, wsegs, hsegs){
      var txtrLoader = new THREE.TextureLoader();
      var self = this;
...
users.TTexturedRectangle.subclass('users.TVideoRectangle',
  'properties',{
    is3D: false,
    tVideo: null,
    height: null
  ...
  'initialize',{
    initialize: function($super, parent, onComplete, tVideo, height, segs){
  ...
  'events',{
    onPointerDown: function(pEvt){this.tVideo.startStop(); return true;},
    onPointerUp: function(pEvt){return true;},
    onPointerMove: function(pEvt){return true;},
    update: function(tm){if(this.tVideo.isPlaying)this.tVideo.texture.needsUpdate = true;},
  }
);
...
var TTrackball = TObject.subclass('users.TTrackball',

  'properties',{
    dragVector: null, // drag position of object
    fromVector: null, // normal vector of the start quaternion vec
    toVector: null, // normal vector of the end quaternion vec
    vec: null, // temp space
    sphere: null, 
    plane: null,
    qSpin: null
...
var TGhost = TObject.subclass('users.TGhost', 
  ...
  'behavior',
  {
    update: function(time, tScene){
      ...
      if (this.isChasing && this.chasing && !this.paused) {
...
var TLazySusan = TObject.subclass('users.TLazySusan',
// spin around the axix of cylinder
  'properties',{
    dragVector: new THREE.Vector3(), // drag position of object
    fromVector: new THREE.Vector3(), // normal vector of the start quaternion vec
    toVector: new THREE.Vector3(), // normal vector of the end quaternion vec
    vec: new THREE.Vector3(), // temp space
    cylinder: null, 
    plane: null,
    rotation: 0,
    isPointerDown: false
  ...
  'initialize',{
    ...
    onPointerUp: function(pEvt){this.isPointerDown = false; return true;},
    onPointerEnter: function(pEvt){console.log('TLazySusan>>onPointerEnter'); return true;},
    onPointerOver: function(pEvt){console.log('TLazySusan>>onPointerOver'); return true;},
    onPointerLeave: function(pEvt){console.log('TLazySusan>>onPointerLeave'); return true;},
  },
  'compute',{
    intersectCylinder: function(ray, cylinder, toVec){
...
var TSlider = TObject.subclass('users.TSlider',
// slider with value of 0-1. When value changes, it calls the action method
  ...
  'initialize',{
    initialize: function(parent, onComplete, action, length, size, initVal){
  ...
  'events',{
    ...
    onPointerUp:function(pEvt){this.plane=null; return true;},
...
var TColorSphere = TObject.subclass('users.TColorSphere',
 'properties',{
   action: null,
   color: null,
   lastColor: null,
 ...
 'initialize',{
   initialize: function($super, parent, onComplete, action, radius){
    ...
    launchColorPicker: function(color){
      var self = this;
      if(!this.colorWheel)
...
var TColorWheel = TObject.subclass('users.TColorWheel',
// color wheel sets a color
  ...
  'initalize',{
    initialize: function(parent, onComplete, action, color, radius){
      this.action = action;
  ...
  'actions',{
    ...
    cancelButton: function(){ this.object3D.visible = false; if(this.action)this.action(null, true);},
    okButton: function(){ this.object3D.visible = false; if(this.action)this.action(this.color, true);},
    ...
    setColor:function(col){console.log(col);},
  ...
  'events',{
    ...
    onPointerMove: function(pEvt){ if(pEvt.selectedTObject == this) return this.onPointerDown(pEvt); return true},
    onPointerUp: function(pEvt){ return true}
  }
);

// starting with Collada, but will generalize this later.
var TLoader = TObject.subclass('users.TLoader',
  'initialize',{
    initialize: function(parent, onComplete, fileName){
