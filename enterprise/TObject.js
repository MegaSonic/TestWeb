...
var TObject = Object.subclass('users.TObject',
  // users.TObject is the base class of the 3D objects in Croquet
    'properties',{
      object3D:null, // THREE.js object
      parent: null, // owning TObject
      goToHere: null,
      selectable: true,
      extent: null,
      plane: null
    ...
    'initializing',{
        initialize: function($super, parent, onComplete, object3D){
   ...
   'accessing',{
      ...
      getObject3D: function(){return this.object3D},

      getRayTestArray: function(){return [this.object3D]; }, // returns an array. If you override this, you need to manage the result in the events
      ...
      withAllChildrenDo: function(doFn, depth) {
        if (!depth) depth = 0;
      ...
      onAdoption: function(){}, // if we are added to a parent

      onOrphan: function(){}, // if we are removed from a parent

      onContentsChanged: function(){}, // if our content changes in some way, usually in size, then do something

      contentsChanged: function(){if(this.parent){this.parent.onContentsChanged(); this.parent.contentsChanged();}}, //let parent know contents have changed
      ...
      position3D: function() { return this.object3D.position; }
    },
    'behavior',{
    ...
    'actions',{
      visible: function(v){if(this.object3D)this.object3D.visible = v;},

      goTo: function(pos, quat, scale, count) {
        if (!this.goToHere)
      ...
      makePlane: function(pEvt, worldDirection){
    ...
    'transformations', {
      spinToQuaternion: function(fromV, toV){
        fromV.normalize();
        toV.normalize();
...
var TScene = TObject.subclass('users.TScene',

      // top of the tree
      'properties', {
        renderer: null,
        isMoving: false,
        selectedTObject: null,
        overTObject: null,
        keyboardTObject: null,
        pointerEvent: null,
        driveMatrix: null,
        ...
        systemButtons: null,
        displayed: null,
        resolutionScale: 1, // current resolution scale value
        frameRateSum: 0, // track the frame
        frameCount: 0, // frame counter
        lastTime: 0, // last time stamp of the update
        cVector: new THREE.Vector3(),
       },
      'initialize',{
        initialize: function(scene, camera, renderer, doSetup){
          this.scene = scene;
      ...
      'accessing',{
        getTScene: function(){return this},
        addSystemButtons: function(buttons){
          this.systemButtons = buttons;
        }
      },

      'updating', {
        update: function(timeStamp, tScene){
          this.frameCount++;
          ...
          {
            this.frameCount = 0;
            if (!(Globals.urlOptions.adaptiveresolution===false)) {
      ...
      'pointer events',{ // return of true terminates search for handler
        onPointerDown: function(pEvt){return true;},
        ...
        onDoubleClick: function(pEvt){return true;},
        setDisplay: function(display){
          if(this.displayed){this.displayed.removeSelf();}
          this.displayed = display;
          return this.displayed.doShow(Globals.tAvatar, function(tObj){tObj.object3D.position.set(0,0,-8); tObj.object3D.quaternion.set(0,0,0,1); });
        },
      ...
      "key events", {
        ...
        onKeyUp: function(pEvt) { this.isMoving = false; return true; },
      ...
      'raw events',{
        ...
        setupEvents: function(domElement) {
          let tScene = this;
        ...
        enterTouchMove: function(evt){console.log('onTouchMove'); console.log(evt);},
        enterTouchStart: function(evt){console.log('onTouchStart'); console.log(evt);},
        enterTouchEnd: function(evt){console.log('onTouchEnd'); console.log(evt);},
        isButtonDown: function(evt, buttonNum){
          if ('buttons' in evt) {
        ...
        isLeftMouseButtonDown: function(evt) {return this.isButtonDown(evt,1)},
        isMiddleMouseButtonDown: function(evt) { return this.isButtonDown(evt,2)},
        isRightMouseButtonDown: function(evt) { if(isSafari)return evt.which===3; else return this.isButtonDown(evt,2); },
        // convert controller event into pointerEvent
        enterControllerEvent: function(cEvt){
          ...
          if(cEvt.appButton){
            if(!cEvt.lastAppButton){
              ...
              if(this.selectedTObject){
                this.controllerRayTest(cEvt, this.selectedTObject);
        ...
        enterControllerDown: function(controller){},
        enterControllerMove: function(controller){},
        enterControllerUp: function(controller){},
        enterControllerSlide: function(controller){},
        enterControllerDblClick: function(controller){},

        enterMouseDown: function(evt) {
        ...
        mouseRayTest: function(evt, tObject, suppress){
        ...
        controllerRayTest: function(cEvt, tObject, suppress){
            ...
            this.rayTest(tObject, suppress);
            if(this.tPointer)this.tPointer.trackLaser(this.pointerEvent);
        },

        rayTest: function(tObject, suppress){
          var targets, target;
        ...
        getRelativeMouseXY: function(evt){
...
var TAvatar = TObject.subclass('users.TAvatar',
      // in order to keep the user upright - move the object3D and orient the camera.
      'properties',{
        camera: null,
      },
      'initialize',{
        initialize: function(parent, onComplete, camera, height){
      ...
      'events',{
        ...
        onPointerOver: function(pEvt){ return true;},
        onPointerLeave: function(pEvt){ return true;},
        onKeyDown: function(pEvt){ return true;},
        onKeyStroke: function(pEvt){ return true;},
        onKeyUp: function(pEvt){ return true;}
      }
...
var TLight = TObject.subclass('users.TLight',
      // top of the tree

      'initialize',{
        initialize: function(parent, onComplete, light){
...
var PointerEvent = Object.subclass('users.PointerEvent',
...
var GoTo = Object.subclass( "users.GoTo",
...
var Cylinder = Object.subclass('users.Cylinder',
  // a right cylinder, used as a picking target. Need to generalize to a conic when I have more time
    'properties',{
      point1: null,
      point2: null,
      radius:null,
