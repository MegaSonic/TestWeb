...
var TWindow = TObject.subclass('users.TWindow',
  'properties',{
  	contents: null,
  	extent: null,
  	originalExtent: null,
    hBorder: 0, // if negative, just offset left border
    vBorder: 0, // if negative, just offset top border
  	scaleVector: null,
  	offset:null,
  	size: null,
  	top: null,
  	...
  	backRect: null,
  	buttons: null,
  	carrying: false, // window connected to camera frame
  	visible: true,
  	title: null,
  	titleRect: null,
  	titleTexture: null
  ...
  'initalizing', {
    initialize: function($super, parent, onComplete, title, size, contents, noBacking, hBorder, vBorder){
    ...
    makeButton: function(color, radius, action, title){
  ...
  'actions',{
    ...
    display: function(){
  	...
  	remove: function(){ this.parent.removeChild(this); return this; },
    setMatrix: function(matrix){this.object3D.setMatrix(matrix);} // used to position the window
  },
...
var TPedestal = TObject.subclass('users.TPedestal',
  'properties',{
  	contents: null,
  	extent: null,
  	extent2: null,
  ...
  'initalizing', {
    initialize: function($super, parent, onComplete, size, contents, noBacking){
    	$super(parent, onComplete);
      var title = 'TPedestal';
      this.object3D.setMatrix = function ( matrix ) {
        this.matrix = matrix;
    ...
    makeButton: function(color, radius, action){
      var geometry = new THREE.SphereBufferGeometry( radius, 16, 16 );
      var material = new THREE.MeshPhongMaterial({color: color.getHex(), emissive: 0x444444});
      var sphere = new THREE.Mesh( geometry, material );
      this.buttons.push(new TButton(this, null, action, sphere, 0));
...
var TTextLabel = TRectangle.subclass('users.TTextLabel',
  'properties',{
    title: null,
    titleTexture: null,
    point: null,
  ...
  'initialize',{
    initialize: function($super, parent, onComplete, title, xDim, yDim, fontHeight){
      xDim = xDim || 16;
      yDim = yDim || 1;
    ...
    replaceText: function(text, x, y, fillStyle) {
...
var TAlertMessage = TTextLabel.subclass('TAlertMessage',
  'initialize', {
    ...
    alert: function(message, time, startTime){
      var self = this;
  ...
  'events',{
    onPointerDown: function(pEvt){this.removeSelf(); return true;}
  }
);
